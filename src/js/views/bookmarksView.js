...
// [10.2.1] :- Choose the recipeContainer as parent element :- 
    _parentElement = document.querySelector('.bookmarks__list');

// [10.2.2] :- Make a private data variable 
    _data; // It will be used to store bookmarks array as data parameter from controller.js(which received from model) from [9.3] 
    render(data){ // render receives recipe data as data parameter from controller.js(which received from model) from [9.3] 
                  // and store this data in this._data 

            // Initailly it will show the text as already decided in index.HTML 
            // When we click on bookmark button then it shows the bookmark 
            // but when we remove all the bookmarks then it shows this renderError() as bookmark array is empty 
        ...
        // [10.2.2.1] 
        this._data = data; // then we store that received bookmarks array into _data of this function 

        // [10.2.2.2] 
        const markup = this._generateMarkup(); // Now use data of bookmarked recipes received and create html element to be displayed into right side bookmark container
        // now markup variable is an HTML string having HTML items list of HTML items of right side bookmarked recipes list received from [10.2.2.extended] 

        // [10.2.2.3]
        this._clear(); // Once we bookmark a recipe then we need to clear container having default results then display bookmarks 

        // [10.2.2.4]
        this._parentElement.insertAdjacentHTML('afterbegin', markup); 
        // then fill parentElement(bookmarkContainer) with markup i.e. list of HTML items of right side bookmarked recipes list 
    }
    ...
    // [10.2.2.5] :- Giving select effect to the that recipe in right side bookmarked recipes list 
    ...
    // When we click on recipe then id in url changes then due to [3.3.2.6] of recipeViews.js, controlRecipe() at [3.5] of controller.js is called 
    ...
    // called bookmarksView.update() here in controlRecipe() at [3.5] of controller.js 
    // bcs it gets called when we click on a recipe from side search result list 
    // then the changed id due to that recipe link can help to update the the same recipe in right side bookmarked recipes list also 
    // In it we also call this bookmarksView.update(state.bookmarks) function of below [10.2.2.5] at [3.5.2] of controller.js and pass bookmarks array in it
    // before rendering resultant recipe on right side (Aise hii)
    // the update() function got called with existing right side bookmarked recipes list and we stored it into this.data  
    // So below In update() We again make a newMarkup using _generateMarkup() using this.data which is same as previous right side bookmarked recipes list 
    // As we clicked on a recipe then id of url becomes the id of clicked recipe as recipes are not buttons but are links 
    // So _generateMarkupPreview() at [10.2.2.2.extended.2] resposible for making items of right side bookmarked recipes list  
    ...
    // Now due to that if check of _generateMarkup() at [10.2.2.2.extended.2] 
    ...
    // Now instead of re-rendering entire right side bookmarked recipes container with newMarkup 
    ...
    update(data){ // Receives existing search results data [3.5.2] of controller.js
      this._data = data; // Store this received data into _data which is at [10.2.2.1] used to generate newMarkup  
    ...
    // [10.2.2.6] :- Loading bookmarks from localStorage with help of contrlBookmark at [11] of controller.js 
    // Due to [2] of model.js savedbookmarks[] array from localStorage will be stored in bookmarks[] array 
    // When page will load this will call controlBookmarks which will render this bookmarks[] array from localStrage into bookmarks section 
    addHandlerRender(handler){ // This handler is controlBookmarks from [11]
      window.addEventListener('load', handler);
    }

    // [10.2.2.7] :- Error Handling in view 
    renderError = function(msg = this._errorMessage){
      ...
      this._clear(); // If we received an empty array bcs no bookmarks then 
      this._parentElement.insertAdjacentHTML('afterbegin', markup); // display error msg 
    }

    // [10.2.2.8] :- Success Message
    ...
    // [10.2.2.2.extended.1] 
    _generateMarkup(){
        // Bascially data is an array of bookmarks 
        // We pass each elements of data i.e. each bookmarked recipe into generateMarkupPreview 
        // and generateMarkupPreview makes each bookmark recipe into an HTML element which is to be an HTML item list of right side bookmarked recipes list
        // and map makes an array of HTML items of right side bookmarked recipes list
        // and then we join that elements of array into a string 
        // i.e. we join all HTML items of right side bookmarked recipes list into a string 
        // then return it to above [10.2.2.2] to markup variable 
        // now markup variable is an HTML string having HTML items of right side bookmarked recipes list
        ...
        // then we display that markup on UI at [10.2.2.4]         
        return this._data.map(this._generateMarkupPreview).join('');
    }

    // [10.2.2.2.extended.2] :- Generating bookmark items for right-side-bookmarks-list
    ...
    // Now, we check if there is key then add user icon on bottom right corner on bookmark in right-side-bookmarks-list 
...
// [10.3] :- Exported to be used in controller 
